...
*MacOSX INFO!* 
Due to a possible bug in a recent Fiji update, if you add this from a Mac the plugins will not appear in your =Plugins= folder. As a workaround, I added the files to the =scripts/Plugins= folder as well, so now 6 files will be added. On a Mac, you will now find the plugins in the main menu at the top of the screen like so:
